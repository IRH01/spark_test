...
object SparkSqlExample1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /** ***************************************************************************************************************
      * 读取数据
      */
    val StudentData = spark.sparkContext.textFile("D:\\dev_data\\idea\\spark_test\\input\\Student").map {
      lines =>
        val line = lines.split(",")
        Row(line(0), line(1), line(2), line(3), line(4))
    }
    val CourseData = spark.sparkContext.textFile("D:\\dev_data\\idea\\spark_test\\input\\Course").map {
      lines =>
        val line = lines.split(",")
        Row(line(0), line(1), line(2))
    }
    val ScoreData = spark.sparkContext.textFile("D:\\dev_data\\idea\\spark_test\\input\\Score").map {
      lines =>
        val line = lines.split(",")
        Row(line(0), line(1), line(2).toInt)
    }
    val TeacherData = spark.sparkContext.textFile("D:\\dev_data\\idea\\spark_test\\input\\Teacher").map {
      lines =>
        val line = lines.split(",")
